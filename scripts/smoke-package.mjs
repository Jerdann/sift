...
const executablePath = path.resolve(
  process.env.SIFT_PACKAGE_DIR ?? process.env.MAIL_STEWARD_PACKAGE_DIR ?? path.join('out', 'Sift-win32-x64'),
  'Sift.exe',
);
const userDataDir = await mkdtemp(path.join(os.tmpdir(), 'mail-steward-package-smoke-'));
let packagedApp;
...
try {
  packagedApp = await electron.launch({
    ...
    env: { ...process.env, SIFT_DISABLE_AUTO_UPDATE: '1' },
  });
  packagedApp.process().stderr?.on('data', (data) => process.stderr.write(data));
  ...
  try {
    await window.waitForFunction(() => Boolean(window.emailOrganizer), undefined, {
      timeout: 10_000,
    });
  } catch (error) {
  ...
  const bridgeRuntime = await window.evaluate(async () => {
    ...
    return {
      ...
      auditMethodsPresent: [
        'startProtonAudit', 'pauseProtonAudit', 'resumeProtonAudit',
        'getCurrentProtonAudit', 'onProtonAuditProgress',
      ].every((method) => typeof window.emailOrganizer[method] === 'function'),
    };
  });
  if (bridgeRuntime.diagnostic.ok || bridgeRuntime.diagnostic.category !== 'bridge_unavailable') {
    throw new Error(`Unexpected packaged Bridge diagnostic: ${bridgeRuntime.diagnostic.category}`);
  }
  if (
