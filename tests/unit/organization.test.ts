...
describe("per-address organization proposals", () => {
  it("separates categories for one domain and revisions every local correction", () => {
    ...
    add(
      ...
      "50% off today",
      { "list-id": "offers.service.example" },
    );
    add(
      "a8d9a12e-5804-4ac0-b12f-6a74e58b1a5d",
    ...
    expect(first.items.map((item) => item.category)).toEqual(
      expect.arrayContaining(["security", "transactions", "promotions"]),
    );
    expect(
    ...
    ).toMatchObject({ messageCount: 2, latestAt: "2025-04-01T10:00:00.000Z" });
    expect(
      first.items.find((item) => item.category === "promotions")?.samples,
    ).toHaveLength(2);

    const promotions = first.items.find(
      (item) => item.category === "promotions",
    )!;
    const edited = proposals.edit({
      proposalId: first.id,
      revision: first.revision,
      itemId: promotions.id,
    ...
    expect(
      edited.items.find((item) => item.id === promotions.id),
    ).toMatchObject({
      category: "subscriptions",
      targetPath: "Primary/Subscriptions/Offers",
      enabled: false,
    ...
    ).toThrow("organization_proposal_changed");
    expect(
      (
        profile.database
          .prepare("SELECT COUNT(*) count FROM organization_corrections")
          .get() as { count: number }
      ).count,
    ).toBe(1);
    ...
    expect(proposals.generate("gmail", connection.id).revision).toBe(
      edited.revision,
    );
