...
} from "../../src/main/gmail/gmail-oauth";
...
describe("Gmail OAuth connection", () => {
  it("uses PKCE token exchange and refresh without putting credentials in URLs", async () => {
    ...
    expect(String(fetchPort.mock.calls[1]![0])).toBe(
      "https://oauth2.googleapis.com/token",
    );
  });

  it("stores refresh access encrypted and isolates it from summaries", () => {
    const root = mkdtempSync(path.join(tmpdir(), "mail-steward-gmail-"));
    roots.push(root);
    const profileId = "881596c7-8676-48d2-90e0-4c64ac402419";
    const profile = new ProfileRepository(root, {
      createId: () => profileId,
    }).createProfile("Gmail owner");
    const vault = new SafeStorageVault(root, profile.database, storage);
    const repository = new GmailConnectionRepository(
      profile.database,
      vault,
      profileId,
      { createId: () => "e39a9273-2842-4b4d-a8ae-e930918176cc" },
    );
    const summary = repository.save(
      { clientId: "synthetic-client-id.apps.googleusercontent.com" },
      "owner@example.test",
      "do-not-render-me",
    );
  ...
  it("indexes Gmail metadata page-by-page and completes without downloading message bodies", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "mail-steward-gmail-audit-"));
    roots.push(root);
    const profileId = "d4273c54-a1c6-4685-987c-d3c49d8e90ec";
    const profile = new ProfileRepository(root, {
      createId: () => profileId,
    }).createProfile("Gmail audit");
    const vault = new SafeStorageVault(root, profile.database, storage);
    const repository = new GmailConnectionRepository(
      profile.database,
      vault,
      profileId,
      { createId: () => "6f647139-7eb2-463b-9055-0ac24d2baf8a" },
    );
    repository.save(
      { clientId: "synthetic-client-id.apps.googleusercontent.com" },
      "owner@example.test",
    ...
    const fetchPort = vi.fn(async (url: string | URL | Request) => {
      ...
      return new Response(
        JSON.stringify({
          ...
          payload: {
            headers: [
              { name: "Subject", value: "Receipt for your payment" },
              { name: "From", value: "Store <billing@store.example>" },
    ...
    const service = new GmailAuditService(profile.database, repository, {
      fetchPort: fetchPort as typeof fetch,
      createId: () => "c08ac9e2-4958-47f6-8cc3-39aa08f0980f",
    });
    ...
    const row = profile.database
      .prepare("SELECT * FROM gmail_indexed_messages")
      .get() as { subject: string; headers_json: string };
    expect(row.subject).toBe("Receipt for your payment");
    expect(row.headers_json).not.toContain("body");
    expect(
      fetchPort.mock.calls.some((call) =>
        String(call[0]).includes("format=metadata"),
      ),
    ).toBe(true);
    profile.database
      .prepare(
        `INSERT INTO gmail_indexed_messages(id,connection_id,gmail_message_id,thread_id,received_at,subject,sender_json,recipients_json,headers_json,label_ids_json,size_bytes,indexed_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
      )
      .run(
        "26965ae0-857c-44dc-b267-7fa9374ae90d",
        repository.get()!.id,
        "gmail-2",
        "thread-2",
    ...
    const analysis = new GmailAnalysisService(
      profile.database,
      profileId,
    ...
    expect(
      unsubscribed.candidates.find(
        (candidate) => candidate.id === eligibleSubscription.id,
      )?.status,
    ).toBe("unsubscribed");
    expect(unsubscribePost).toHaveBeenCalledWith(
    ...
    const organization = new GmailOrganizationRepository(
      profile.database,
      jobs,
      profileId,
    );
    ...
    const approved = organization.approve(
      repository.get()!.id,
      plan.id,
      plan.revision,
    );
    const providerLabels = new Map<string, string[]>([
      ["gmail-1", ["INBOX", "UNREAD"]],
      ["gmail-2", ["INBOX", "UNREAD"]],
    ]);
    ...
    const actionFetch = vi.fn(
      async (url: string | URL | Request, init?: RequestInit) => {
        ...
        if (value.endsWith("/labels") && !init?.method)
        ...
        if (value.includes("/messages/") && value.includes("?format=minimal")) {
          const id = decodeURIComponent(
            value.split("/messages/")[1]!.split("?")[0]!,
          );
          return new Response(
            JSON.stringify({ id, labelIds: providerLabels.get(id) ?? [] }),
            { status: 200 },
          );
        }
          ...
          for (const id of body.ids)
            providerLabels.set(
              ...
              [
                ...
                .filter((label) => !body.removeLabelIds.includes(label))
                .sort(),
            );
          if (failAfterNextModify) {
            failAfterNextModify = false;
    ...
    await new GmailOrganizationRunner(
      repository,
      organization,
      jobs,
    ...
    const batchCall = actionFetch.mock.calls.find((call) =>
      String(call[0]).endsWith("/messages/batchModify"),
    )!;
    ...
    expect(
      actionFetch.mock.calls.some((call) =>
        String(call[0]).includes("/settings/filters"),
      ),
    ).toBe(false);
    const undoPlan = organization.prepareUndo(completed.id);
    const undone = await new GmailOrganizationRunner(
      repository,
      organization,
      jobs,
      actionFetch as typeof fetch,
    ).undo(undoPlan.undoJob!.id);
    ...
    const crashApproved = organization.approve(
      repository.get()!.id,
      crashPlan.id,
      crashPlan.revision,
    );
    failAfterNextModify = true;
    const firstAttempt = await new GmailOrganizationRunner(
      repository,
      organization,
      jobs,
      actionFetch as typeof fetch,
    ).run(crashApproved.job!.id);
    expect(firstAttempt.failedBatches).toHaveLength(1);
    const modifyCount = actionFetch.mock.calls.filter((call) =>
      String(call[0]).endsWith("/messages/batchModify"),
    ).length;
    const retried = organization.retry(
      firstAttempt.id,
      firstAttempt.failedBatches.map((batch) => batch.id),
    );
    const recovered = await new GmailOrganizationRunner(
      repository,
      organization,
      jobs,
      actionFetch as typeof fetch,
    ...
    expect(
      actionFetch.mock.calls.filter((call) =>
        String(call[0]).endsWith("/messages/batchModify"),
      ),
    ).toHaveLength(modifyCount);

    const trashRepository = new GmailOrganizationRepository(
      profile.database,
      jobs,
      profileId,
    ...
    const trashPlan = trashRepository.generate(repository.get()!, {
      kind: "trash",
      senderDomains: ["news.example", "store.example"],
      olderThanDays: 180,
    });
    expect(trashPlan).toMatchObject({
      kind: "trash",
      state: "draft",
      impactCount: 1,
      existingMessageCount: 1,
    });
    expect(trashPlan.impacts[0]).toMatchObject({
      sourceCategory: "newsletters",
      targetLabel: "TRASH",
      trash: true,
    });
    expect(
      trashPlan.impacts.some(
        (impact) => impact.sourceCategory === "transactions",
      ),
    ).toBe(false);
    const trashApproved = trashRepository.approve(
      repository.get()!.id,
      trashPlan.id,
      trashPlan.revision,
    );
    const trashCompleted = await new GmailOrganizationRunner(
      repository,
      trashRepository,
      jobs,
      actionFetch as typeof fetch,
    ).run(trashApproved.job!.id);
    ...
    const trashUndone = await new GmailOrganizationRunner(
      repository,
      trashRepository,
      jobs,
      actionFetch as typeof fetch,
    ).undo(trashUndo.undoJob!.id);
    ...
    const newsCandidate = spamReview.candidates.find(
      (candidate) => candidate.senderDomain === "news.example",
    )!;
    ...
    const spamApproved = organization.approve(
      repository.get()!.id,
      spamPlan.id,
      spamPlan.revision,
    );
    const spamCompleted = await new GmailOrganizationRunner(
      repository,
      organization,
      jobs,
      actionFetch as typeof fetch,
    ).run(spamApproved.job!.id);
