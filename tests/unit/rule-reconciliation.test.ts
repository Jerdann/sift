...
} from "../../src/shared/contracts/rule-management";
...
const setup = () => {
  ...
  add(
    "72ea544e-8999-461d-8bde-287f03e1b2b1",
    "mail@offers.example",
    "50% off today",
    { "list-id": "offers.example" },
  );
  add(
    "ce34d6aa-811d-4a99-9e02-0cc95e404834",
    "mail@offers.example",
    "Last chance sale",
    { "list-id": "offers.example" },
  );
  add(
    "aa34d6aa-811d-4a99-9e02-0cc95e404835",
    "mail@offers.example",
    "Clearance coupon",
    { "list-id": "offers.example" },
  );
  add(
    "ba34d6aa-811d-4a99-9e02-0cc95e404836",
    "orders@shop.example",
    "Your order confirmation",
  );
  add(
    "ca34d6aa-811d-4a99-9e02-0cc95e404837",
    "orders@shop.example",
    "Your order receipt",
  );
  add(
    "da34d6aa-811d-4a99-9e02-0cc95e404838",
    "orders@shop.example",
    "Payment confirmed",
  );
  add(
    "afafc196-49af-4ec5-8d67-62aa05295662",
    "alerts@secure.example",
...
describe("provider rule inventory and reconciliation", () => {
  it("normalizes Gmail filter ordering into one semantic fingerprint", () => {
    ...
    const left = normalizeGmailFilter(
      {
        id: "filter-left",
        criteria: { from: " @OFFERS.EXAMPLE ", to: "OWNER@EXAMPLE.TEST" },
        action: {
          addLabelIds: ["Label_2"],
    ...
    const right = normalizeGmailFilter(
      {
        id: "filter-right",
        criteria: { to: "owner@example.test", from: "@offers.example" },
        action: {
  ...
  it("adopts an exact external match, creates missing rules, and never plans against unrelated filters", () => {
    ...
    expect(plan.operations.map((operation) => operation.kind).sort()).toEqual([
      "adopt",
      "create",
      "create",
    ]);
    expect(
      plan.operations.find((operation) => operation.kind === "adopt")?.prior
        ?.providerRuleId,
    ).toBe("existing-exact");
    expect(
      plan.operations.some(
        (operation) => operation.prior?.providerRuleId === "external-unrelated",
      ),
    ).toBe(false);
  ...
  it("does not create a filing rule for repeated unsorted mail", () => {
    ...
    for (let index = 0; index < 3; index += 1) {
      ...
      insert.run(
        ...
        connection.id,
        id,
        `thread-unsorted-${index}`,
        "2026-08-25T10:00:00.000Z",
        `Generic update ${index}`,
    ...
    const desired = rules
      .desired("gmail", connection.id)
      .rules.find((rule) => rule.senderDomain === "company.example");
    expect(desired).toBeUndefined();
    profile.database.close();
  });

  it("does not invent a company-wide Spam rule from an aggregate without matching message evidence", () => {
    const { profile, profileId, connection, rules } = setup();
    const analysis = profile.database
      .prepare("SELECT id FROM gmail_mailbox_analyses WHERE connection_id=?")
    ...
    `,
      ...
      .run(
        "9d0a10ef-1ef8-4c17-9472-30d8fa69e351",
        analysis.id,
        "junk.example",
        "spam",
        "owner@example.test",
    ...
    const candidate = review.candidates.find(
      (item) => item.senderDomain === "junk.example",
    )!;
    reviews.complete({
      reviewId: review.id,
    ...
    expect(rules.desired("gmail", connection.id).rules).not.toContainEqual(
      expect.objectContaining({
  ...
  it("uses the latest mailbox scan when the folder proposal points to an older scan", () => {
    ...
    for (let index = 0; index < 3; index += 1) {
      ...
      insert.run(
        ...
        connection.id,
        id,
        `thread-second-offer-${index}`,
        "2026-08-25T11:00:00.000Z",
        `Another limited sale ${index}`,
        '["mail@second-offers.example"]',
        '["owner@example.test"]',
    ...
    expect(
      desired.some((rule) => rule.senderDomain === "second-offers.example"),
    ).toBe(true);
    profile.database.close();
  });
  ...
  it("uses a stable identity when a correction requires replacing a managed rule", () => {
    const { profile, profileId, connection, rules } = setup();
    const firstDesired = rules
      .desired("gmail", connection.id)
      .rules.find((rule) => rule.category === "promotions")!;
    ...
    profile.database
      ...
      .run(
        "59ba97f-e105-44ce-9341-c9997e1ae9d2",
        profileId,
        "gmail",
        connection.id,
        firstDesired.stableKey,
        "managed-filter-1",
        firstDesired.fingerprint,
        JSON.stringify(firstDesired),
        "managed",
        now,
        now,
      );
    rules.saveInventory(
      ...
      connection.id,
      "live_api",
      [providerSnapshot("managed-filter-1", firstDesired)],
      1_000,
    );
    ...
    new OrganizationProposalRepository(profile.database, profileId).edit({
      ...
      targetPath: "Primary/Subscriptions/Offers",
      enabled: true,
    });
    const corrected = rules
      .desired("gmail", connection.id)
      .rules.find((rule) => rule.senderDomain === firstDesired.senderDomain)!;
    expect(corrected.stableKey).toBe(firstDesired.stableKey);
    expect(corrected.fingerprint).not.toBe(firstDesired.fingerprint);
    const plan = rules.generate("gmail", connection.id);
    expect(
      plan.operations.find(
        (operation) => operation.stableKey === firstDesired.stableKey,
      )?.kind,
    ).toBe("replace");
    profile.database.close();
  });
  ...
  it("reads Gmail labels and filters without mutating provider state", async () => {
    ...
    const fetchPort = vi.fn(
      async (input: string | URL | Request, init?: RequestInit) => {
        ...
        if (url.endsWith("/labels"))
          return new Response(
            JSON.stringify({
              labels: [{ id: "Label_1", name: "Primary/Promotions" }],
            }),
          );
    ...
    const inventory = await new GmailRuleInventoryService(
      connections,
      rules,
      fetchPort,
    ...
    expect(inventory.rules[0]).toMatchObject({
      providerRuleId: "provider-filter-1",
      ownership: "external",
    });
    expect(inventory.rules[0]?.action.addLabels).toEqual([
      "Primary/Promotions",
    ]);
    expect(inventory.containers).toEqual(["Primary/Promotions"]);
    const providerReads = fetchPort.mock.calls.filter((call) =>
      String(call[0]).includes("gmail.googleapis.com"),
    );
    expect(
      providerReads.every(
  ...
  it("accepts live Proton folders when a rescan removed the completed Organize record", () => {
    const root = mkdtempSync(path.join(tmpdir(), "sift-proton-folders-"));
    roots.push(root);
    const profileId = "9c1c28a1-5d31-458a-9de3-f10ca5d68e9c";
    const profile = new ProfileRepository(root, {
      createId: () => profileId,
    }).createProfile("Folder owner");
    const vault = new SafeStorageVault(root, profile.database, storage);
    ...
    `,
      ...
      .run(
        "8d0768a0-189f-49f2-883d-ab9a2cf659d3",
        connection.id,
        profileId,
        "Folders/Primary/Promotions",
        "Promotions",
      );
    const desired: DesiredManagedRule = {
      stableKey: "a".repeat(64),
      fingerprint: "b".repeat(64),
      senderDomain: "offers.example",
      receivingAddress: null,
      category: "promotions",
      targetPath: "Primary/Promotions",
      markRead: true,
      archive: false,
      spam: false,
      observedMessages: 5,
    ...
    const plan = {
      id: "8e4df6a8-8113-4a51-b9aa-c5dc5ec98c64",
      provider: "proton",
      connectionId: connection.id,
      proposalId: "0f02e04d-a763-477c-a1d1-8eff343ac5e2",
      proposalRevision: "c".repeat(64),
      spamReviewId: "1f02e04d-a763-477c-a1d1-8eff343ac5e3",
      inventoryId: "479e1574-d90d-437e-9e4a-d30921a7edc6",
      ...
      operations: [
        {
          id: "3740aa88-e30f-42dd-af68-12316d5b0d29",
    ...
    profile.database
      .prepare("DELETE FROM mail_containers WHERE connection_id=?")
      .run(connection.id);
  ...
  it("resumes after a provider create without duplicating the Gmail filter", async () => {
    ...
    const proposalRepository = new OrganizationProposalRepository(
      profile.database,
      profileId,
    );
    const proposal = proposalRepository.get("gmail", connection.id)!;
    const security = proposal.items.find(
    ...
    expect(() =>
      rules.approve(draft.id, draft.revision, [selectedOperation.id]),
    ).toThrow("organization_folders_required");
    ...
    `,
      ...
      .run(
        "df77dc85-7ef8-40ba-8b52-5acba8d9a119",
        connection.id,
        "completed-history-revision",
        "2026-08-25T12:25:00.000Z",
        "2026-08-25T12:26:00.000Z",
        appliedProposal.id,
        appliedProposal.revision,
        connection.id,
      );
      ...
      async (input: string | URL | Request, init?: RequestInit) => {
        ...
        if (url.endsWith("/labels") && !init?.method)
          return new Response(
            JSON.stringify({
              labels: [
                { id: "Label_1", name: selectedOperation.desired!.targetPath },
              ],
            }),
          );
    ...
    expect(filters).toHaveLength(1);
    expect(
      fetchPort.mock.calls.filter(
        (call) =>
          String(call[0]).endsWith("/settings/filters") &&
          call[1]?.method === "POST",
      ),
    ).toHaveLength(1);
    expect(
      rules.managedRule("gmail", connection.id, selectedOperation.stableKey),
    ).toMatchObject({ provider_rule_id: "created-filter-1", state: "active" });
