...
describe("local mailbox analysis", () => {
  it.each([
    ["Your verification code is 123456", {}, "codes"],
    ...
    ["Weekly engineering digest", { "list-id": "news.example" }, "newsletters"],
    [
  ...
  it("deduplicates folder copies and builds category, stream, and address-service proposals", () => {
    ...
    const discovery = new ProtonDiscoveryRepository(
      ...
      addresses: [
        ...
        {
          address: "home@pm.test",
          occurrenceCount: 1,
          lastSeenAt: "2026-08-24T12:30:00.000Z",
          sources: ["delivered-to"],
        },
        {
          address: "coworker@company.test",
          occurrenceCount: 1,
          lastSeenAt: "2026-08-24T13:00:00.000Z",
          sources: ["to"],
        },
      ],
    });
    ...
    `);
    const inbox = discovery.mailboxes.find(
      (mailbox) => mailbox.path === "INBOX",
    )!;
    const all = discovery.mailboxes.find(
      (mailbox) => mailbox.path === "All Mail",
    )!;
    const sent = discovery.mailboxes.find(
      (mailbox) => mailbox.path === "Sent",
    )!;
    insert.run(
      "fa2570e3-bdef-4d41-8931-301ba7185801",
      connection.id,
      inbox.id,
      "1",
      1,
    ...
    insert.run(
      "69410e65-1112-4d92-af72-10c19039db5e",
      connection.id,
      all.id,
      "2",
      1,
    ...
    insert.run(
      "87fddcf8-b182-4f41-92e4-88007126b2bb",
      connection.id,
      inbox.id,
      "1",
      2,
    ...
    insert.run(
      "a6efc99d-5558-4512-b370-a7ebde40c3b5",
      connection.id,
      inbox.id,
      "1",
      3,
    ...
    insert.run(
      "6a2f4af1-c2c9-4525-9710-712d5ea9bd5e",
      connection.id,
      inbox.id,
      "1",
      4,
      ...
      '["home@pm.test"]',
      '{"delivered-to":"home@pm.test","x-original-to":"home=pm.test+partner=mail.test@forward.protonmail.ch"}',
    );
    insert.run(
      "f7aa8027-636f-4a7c-91d4-9e206028f8fd",
      connection.id,
      sent.id,
      "3",
      1,
      ...
      "Project question",
      '["owner@pm.test"]',
      '["coworker@company.test"]',
      "{}",
    );

    const unreviewed = analyzeMailbox(
      profile.database,
      profileId,
      connection.id,
    );
    ...
    expect(
      identities
        .list("proton", connection.id)
        .map((identity) => identity.address),
    ).toEqual(["home@pm.test", "owner@pm.test"]);
    for (const identity of identities.list("proton", connection.id)) {
      identities.update({
        ...
        status: "confirmed",
        containerEnabled: identity.address === "home@pm.test",
    ...
    `,
        ...
        .get("fa2570e3-bdef-4d41-8931-301ba7185801"),
    ).toEqual({ path: "INBOX" });
    expect(
      result.categories.map((item) => [item.category, item.messageCount]),
    ).toEqual(
    ...
    expect(result.addresses.map((address) => address.address)).toEqual([
      "owner@pm.test",
      "home@pm.test",
    ]);
    expect(
      result.topStreams.every((stream) =>
        ["owner@pm.test", "home@pm.test"].includes(stream.receivingAddress),
      ),
    ).toBe(true);
    ...
    expect(result.addresses[0]).toMatchObject({
      ownershipEvidence: "sent_and_received",
      canRetire: true,
    });
    expect(result.addresses[1]).toMatchObject({
      address: "home@pm.test",
      ownershipEvidence: "received",
      canRetire: false,
    });
    expect(result.addresses[1]).toMatchObject({
    ...
    expect(
      result.addresses[0]!.services.map((service) => service.domain),
    ).toEqual(expect.arrayContaining(["store.example", "service.example"]));
    const pack = buildPortableRulePack(result);
    expect(pack.rules).toEqual([]);
    expect(() => renderProtonSieve(pack)).toThrow(
      "review_purpose_filters_in_rules",
    );
