...
export const accountProviderSchema = z.enum(["proton", "gmail", "outlook"]);
...
export const accountSelectionInputSchema = z
  .object({
    provider: accountProviderSchema,
    connectionId: z.uuid(),
  })
...
export const accountIdentityUpdateInputSchema = z
  .object({
    provider: accountProviderSchema,
    connectionId: z.uuid(),
    address: z.email(),
  ...
  .superRefine((input, context) => {
    if (input.containerEnabled && input.status !== "confirmed") {
      context.addIssue({
        code: "custom",
        path: ["containerEnabled"],
        message: "Only confirmed addresses can use a container",
      });
    }
    if (input.containerEnabled && !input.containerName) {
      context.addIssue({
        code: "custom",
        path: ["containerName"],
        message: "A container name is required",
      });
    }
  });
...
export type IdentityEvidenceSource = z.infer<
  typeof identityEvidenceSourceSchema
>;
...
export type AccountIdentityListInput = z.infer<
  typeof accountIdentityListInputSchema
>;
export type AccountIdentitySummary = z.infer<
  typeof accountIdentitySummarySchema
>;
export type AccountIdentityUpdateInput = z.infer<
  typeof accountIdentityUpdateInputSchema
>;
