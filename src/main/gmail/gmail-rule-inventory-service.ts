...
export class GmailRuleInventoryService {
  ...
  async refresh(connectionId: string): Promise<RuleInventory> {
    ...
    const [labelPayload, filterPayload] = await Promise.all([
      gmailApi<{ labels?: Array<{ id: string; name: string }> }>(
        this.#fetchPort,
        token,
        "https://gmail.googleapis.com/gmail/v1/users/me/labels",
      ),
      gmailApi<{ filter?: GmailFilterResource[] }>(
        this.#fetchPort,
        token,
    ...
    const labelNames = new Map(
      (labelPayload.labels ?? []).map((label) => [label.id, label.name]),
    );
    const snapshots = (filterPayload.filter ?? []).map((filter) =>
      normalizeGmailFilter(filter, labelNames),
    );
