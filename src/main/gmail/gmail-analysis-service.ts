...
export class GmailAnalysisService {
  ...
  analyze(connection: GmailConnectionSummary): MailboxAnalysisSummary {
    ...
    const classified = rows
      .filter(
        (row) =>
          !stringArray(row.label_ids_json).some(
    ...
    const addresses = identities
      .map((identity) => {
        ...
        for (const stream of addressStreams) {
          ...
          const current = services.get(stream.sender_domain) ?? {
            messageCount: 0,
            latestAt: null,
          ...
          if (
            stream.latest_at &&
            (!current.latestAt || stream.latest_at > current.latestAt)
          )
            current.latestAt = stream.latest_at;
