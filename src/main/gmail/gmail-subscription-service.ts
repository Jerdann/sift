...
export class GmailSubscriptionService {
  ...
  scan(connectionId: string): SubscriptionDashboard {
    ...
    for (const row of rows) {
      ...
      for (const address of addresses.length ? addresses : ["unknown"]) {
        ...
        if (!(JSON.parse(row.label_ids_json) as string[]).includes("UNREAD"))
          group.readCount += 1;
        if (
          row.subject &&
    ...
    this.#database.transaction(() => {
      this.#database
        .prepare("DELETE FROM gmail_subscription_scans WHERE analysis_id=?")
        .run(analysisId);
      this.#database
        .prepare(
          "INSERT INTO gmail_subscription_scans(id,analysis_id,profile_id,generated_at) VALUES (?,?,?,?)",
        )
        .run(scanId, analysisId, this.#profileId, generatedAt);
      const add = this.#database.prepare(
        "INSERT INTO gmail_subscription_candidates(id,scan_id,sender_domain,list_id,receiving_address,endpoint,eligibility,authenticated,message_count,latest_at,categories_json,sample_subjects_json,status,reason,earliest_at,read_count) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
      );
  ...
  getCurrent(connectionId: string): SubscriptionDashboard | null {
    const row = this.#database
      .prepare(
        "SELECT gss.id FROM gmail_subscription_scans gss JOIN gmail_mailbox_analyses gma ON gma.id=gss.analysis_id WHERE gma.connection_id=? AND gss.profile_id=? ORDER BY gss.rowid DESC LIMIT 1",
      )
  ...
  getByScan(scanId: string): SubscriptionDashboard {
    const scan = this.#database
      .prepare(
        "SELECT * FROM gmail_subscription_scans WHERE id=? AND profile_id=?",
      )
    ...
    const connection = this.#database
      .prepare(
        "SELECT gma.connection_id FROM gmail_subscription_scans gss JOIN gmail_mailbox_analyses gma ON gma.id=gss.analysis_id WHERE gss.id=?",
      )
      .get(scanId) as { connection_id: string };
    const rows = this.#database
      .prepare("SELECT * FROM gmail_subscription_candidates WHERE scan_id=?")
      .all(scanId) as Array<Record<string, unknown>>;
    const ledgerRows = this.#database
      .prepare(
        "SELECT list_id,receiving_address,requested_at FROM unsubscribe_ledger WHERE profile_id=? AND provider='gmail' AND connection_id=?",
      )
    ...
    const jobRow = this.#database
      .prepare(
        "SELECT job_id FROM gmail_unsubscribe_runs WHERE scan_id=? ORDER BY rowid DESC LIMIT 1",
      )
  ...
  start(candidateIds: readonly string[]): SubscriptionDashboard {
    ...
    const job = this.#jobs.createJob({
      ...
      idempotencyKey: `gmail-unsubscribe:${scanId}:${[...candidateIds].sort().join(",")}`,
      itemKeys: rows.map((row) => row.id),
    });
    this.#database
      .prepare(
        "INSERT OR IGNORE INTO gmail_unsubscribe_runs(job_id,scan_id,created_at) VALUES (?,?,?)",
      )
  ...
  mark(candidateId: string, status: "unsubscribed" | "failed"): void {
    this.#database.transaction(() => {
      this.#database
        .prepare("UPDATE gmail_subscription_candidates SET status=? WHERE id=?")
  ...
  retry(jobId: string, candidateIds: readonly string[]): SubscriptionDashboard {
    ...
    this.#database
      .prepare(
        `UPDATE gmail_subscription_candidates SET status='pending' WHERE scan_id=? AND id IN (${placeholders})`,
      )
  ...
  scanIdForJob(jobId: string): string {
    const row = this.#database
      .prepare("SELECT scan_id FROM gmail_unsubscribe_runs WHERE job_id=?")
