...
import type {
  GmailHistoryBatchRecord,
  GmailOrganizationRepository,
} from "./gmail-organization-repository";
...
const mapMatches = (
  ...
  ids: readonly string[],
): boolean => ids.every((id) => same(actual[id], expected[id]));
...
export class GmailOrganizationRunner {
  ...
  constructor(
    ...
    plans: GmailOrganizationRepository,
    jobs: JobRepository,
  ...
  async run(
    jobId: string,
    onProgress?: (
      plan: ReturnType<GmailOrganizationRepository["sync"]>,
    ) => void,
  ) {
    ...
    for (;;) {
      ...
      try {
        ...
        const targetLabelId = batch.trash
          ? "TRASH"
          : batch.spam
            ? "SPAM"
            : await this.#labelId(token, batch.targetLabel, labels);
        const desired = Object.fromEntries(
          batch.messageIds.map((id) => {
            const next = new Set(batch.priorLabels[id] ?? []);
            next.add(targetLabelId);
            if (batch.archive || batch.spam || batch.trash)
              next.delete("INBOX");
        ...
        if (!mapMatches(current, desired, batch.messageIds)) {
          if (!mapMatches(current, batch.priorLabels, batch.messageIds))
        ...
        if (!mapMatches(verified, desired, batch.messageIds))
          throw new Error("provider_verification_mismatch");
        ...
        this.#jobs.transitionItem(item.id, "succeeded", {
          result: { operation: "gmail-history-batch", verified: true },
        });
      } catch (error) {
        const code =
          error instanceof Error ? error.message : "gmail_history_failed";
        const state =
  ...
  async undo(
    ...
    for (;;) {
      ...
      try {
        if (!batch.resultingLabels)
          throw new Error("gmail_history_undo_receipt_missing");
        const token = await this.#token(batch.connectionId, tokens);
        const current = await this.#readLabels(token, batch.messageIds);
        if (!mapMatches(current, batch.resultingLabels, batch.messageIds))
          throw new Error("provider_verification_mismatch");
        ...
        for (const id of batch.messageIds) {
          ...
          const add = [...prior]
            .filter((label) => !resulting.has(label))
            .sort();
          const remove = [...resulting]
            .filter((label) => !prior.has(label))
            .sort();
        ...
        for (const group of groups.values())
          await this.#batchModify(token, group.ids, group.add, group.remove);
        const verified = await this.#readLabels(token, batch.messageIds);
        if (!mapMatches(verified, batch.priorLabels, batch.messageIds))
          throw new Error("provider_verification_mismatch");
        this.#plans.syncIndexedLabels(batch.connectionId, verified);
        ...
        const code =
          error instanceof Error ? error.message : "gmail_history_undo_failed";
        const state =
          code === "provider_verification_mismatch"
            ? "verification_mismatch"
            : "failed";
    ...
    for (let offset = 0; offset < ids.length; offset += 10) {
      await Promise.all(
        ids.slice(offset, offset + 10).map(async (id) => {
          const message = await api<{ id: string; labelIds?: string[] }>(
            this.#fetchPort,
            token,
