...
export class CleanupPlanRepository {
  ...
  generate(connectionId: string, input: GenerateCleanupInput): CleanupPlan {
    ...
    const preferencesFor = (candidate: CandidateRow) =>
      handling.resolve(
        "proton",
        connectionId,
        (
    ...
    const proposal =
      input.kind === "organize"
        ? (this.#database
            .prepare(
              "SELECT id,revision FROM organization_proposals WHERE profile_id=? AND provider='proton' AND connection_id=? AND state='draft' ORDER BY updated_at DESC,rowid DESC LIMIT 1",
            )
            .get(this.#profileId, connectionId) as
            { id: string; revision: string } | undefined)
        : undefined;
    if (input.kind === "organize" && !proposal)
    ...
    for (const item of proposalItems)
      proposalBySource.set(item.source_category, [
        ...(proposalBySource.get(item.source_category) ?? []),
        item,
      ]);
    const proposedItem = (candidate: CandidateRow): ProposalItemRow | null => {
      if (input.kind !== "organize") return null;
      const receiving = new Set(
        (JSON.parse(candidate.receiving_addresses_json) as string[]).map(
      ...
      return (
        (proposalBySource.get(candidate.category) ?? [])
          .filter(
            (item) =>
              !item.scope_address ||
              receiving.has(item.scope_address.toLowerCase()),
          )
          .sort(
            (left, right) =>
              Number(Boolean(right.scope_address)) -
                Number(Boolean(left.scope_address)) ||
              (left.scope_address ?? "").localeCompare(
                right.scope_address ?? "",
              ),
          )[0] ?? null
      );
    };
    const excludedContainerRoles = new Set([
      "\\all",
      "\\sent",
      "\\drafts",
      "\\trash",
    ...
    const trashDomains = new Set(
      input.trashSenderDomains.map((domain) => domain.toLowerCase()),
    );
    const actionable = candidates.filter((candidate) => {
      if (
        new Set(JSON.parse(candidate.receiving_addresses_json) as string[])
          .size !== 1
      )
        return false;
      const roles = new Set([
        candidate.special_use?.toLowerCase() ?? "",
        ...(JSON.parse(candidate.container_flags_json) as string[]).map(
          (flag) => flag.toLowerCase(),
        ),
      ]);
      if ([...roles].some((role) => excludedContainerRoles.has(role)))
        return false;
      if (input.kind === "spam") {
        if (
          !removableCategories.has(candidate.category) &&
          candidate.category !== "spam"
        )
          return false;
        const receiving = JSON.parse(
          candidate.receiving_addresses_json,
        ) as string[];
      ...
      if (input.kind === "trash") {
        if (input.retention)
          return retentionEligible(
            preferencesFor(candidate),
            candidate.category,
        ...
        return (
          trashDomains.has(candidate.sender_domain.toLowerCase()) &&
          removableCategories.has(candidate.category) &&
          candidate.confidence >= 0.82 &&
          Boolean(candidate.received_at) &&
          Date.parse(candidate.received_at!) <
            Date.parse(this.#now()) - 180 * 86400000
        );
      }
      const item = proposedItem(candidate);
      // The proposal and the per-type policy jointly constrain each approved
      // historical action. Read status is independent of folder selection.
      return Boolean(
        item?.enabled &&
        handlingEligible(
          preferencesFor(candidate),
        ...
        handlingFor(preferencesFor(candidate), item.category).destination !==
          "inbox" &&
        (!["spam", "trash"].includes(
          handlingFor(preferencesFor(candidate), item.category).destination,
        ) ||
    ...
    const actionValues = actionable
      .map((candidate) => {
        ...
        const destination =
          input.kind === "organize" ? policy.destination : input.kind;
        const categoryPath =
          item?.target_path ?? CATEGORY_PRESENTATION[effectiveCategory].folder;
        const scopeAddress =
          input.kind === "spam"
            ? ((JSON.parse(candidate.receiving_addresses_json) as string[])
        ...
        return {
          ...
          actionKind:
            destination === "trash"
              ? ("native_trash" as const)
              : destination === "spam"
                ? ("native_spam" as const)
                : ("sort_read_archive" as const),
          markRead:
            input.kind === "organize" ? policy.markRead : input.kind === "spam",
        };
      })
      .filter(
        (action) =>
          action.actionKind !== "sort_read_archive" ||
          !isSameProtonFolder(action.source_path, action.targetPath) ||
          (action.markRead &&
            !(JSON.parse(action.message_flags_json) as string[]).includes(
              "\\Seen",
            )),
      );
    const targetPaths = proposalItems
      .filter(
        (item) =>
          item.enabled &&
          handlingFor(
    ...
    const legacyContainers =
      existingSetup === "replace"
        ...
        `,
            ...
            const parts = container.provider_container_id
              .split(delimiter)
              .filter(Boolean);
            const root = parts[0]?.toLowerCase();
            if (!["folders", "labels"].includes(root ?? "") || parts.length < 2)
              return [];
            const providerTargets = targetPaths.map((target) =>
              protonFolderPath(target, delimiter).toLowerCase(),
            );
            const path = container.provider_container_id.toLowerCase();
            if (
              providerTargets.some(
                (target) =>
                  target === path ||
                  target.startsWith(`${path.toLowerCase()}${delimiter}`),
              )
            )
              return [];
            return [
              {
                ...
                kind:
                  root === "labels" ? ("label" as const) : ("folder" as const),
    ...
    const revision = createHash("sha256")
      .update(
        JSON.stringify({
          ...
          legacyContainers: legacyContainers.map((container) => [
            container.providerPath,
            container.observedMessages,
          ]),
        }),
      )
      .digest("hex");
    ...
    this.#database.transaction(() => {
      this.#database
        .prepare(
          "DELETE FROM cleanup_plans WHERE connection_id = ? AND plan_kind = ? AND state = 'draft'",
        )
        .run(connectionId, input.kind);
      this.#database
        .prepare(
          `
      ...
      `,
        ...
        .run(
          ...
          analysisId,
          revision,
          candidates.length - actionValues.length,
          timestamp,
          input.kind,
          proposal?.id ?? null,
    ...
    handling.stampPlan("cleanup_plans", planId, "proton", connectionId);
    return this.get(planId);
  }

  getCurrent(
    connectionId: string,
    kind: CleanupPlan["kind"] = "organize",
  ): CleanupPlan | null {
    const row = this.#database
      .prepare(
        "SELECT id FROM cleanup_plans WHERE connection_id = ? AND plan_kind = ? ORDER BY rowid DESC LIMIT 1",
      )
      .get(connectionId, kind) as { id: string } | undefined;
  ...
  planIdForJob(jobId: string): string {
    const row = this.#database
      .prepare(
        "SELECT id FROM cleanup_plans WHERE job_id = ? OR undo_job_id = ?",
      )
  ...
  get(planId: string): CleanupPlan {
    ...
    `,
      ...
      | {
          ...
          revision: string;
          state: CleanupPlan["state"];
          plan_kind: CleanupPlan["kind"];
          proposal_id: string | null;
          proposal_revision: string | null;
          existing_setup: "extend" | "reuse" | "replace";
          spam_review_id: string | null;
          skipped_count: number;
          job_id: string | null;
          undo_job_id: string | null;
          created_at: string;
          approved_at: string | null;
          handling_revision: string | null;
        }
      | undefined;
    if (!row) throw new Error("Cleanup plan was not found");
    ...
    `,
      ...
      category: MailCategory;
      target_path: string;
      action_kind: "sort_read_archive" | "native_spam" | "native_trash";
      message_count: number;
    }>;
    const actionCount = impacts.reduce(
      (sum, impact) => sum + impact.message_count,
      0,
    );
    ...
    return cleanupPlanSchema.parse({
      ...
      actionCount,
      spamCount: impacts
        .filter((impact) => impact.action_kind === "native_spam")
        .reduce((sum, impact) => sum + impact.message_count, 0),
      trashCount: impacts
        .filter((impact) => impact.action_kind === "native_trash")
        .reduce((sum, impact) => sum + impact.message_count, 0),
      skippedCount: row.skipped_count,
      requiresRebuild,
      impacts: impacts.map((impact) => ({
        markRead: Boolean(
          (impact as typeof impact & { mark_read: number }).mark_read,
        ),
      ...
      `,
          ...
          id: string;
          target_path: string;
          state: "failed" | "verification_mismatch";
  ...
  approve(planId: string, revision: string): CleanupPlan {
    ...
    const planScope = this.#database
      .prepare(
        "SELECT connection_id,plan_kind,proposal_id,proposal_revision FROM cleanup_plans WHERE id=?",
      )
      .get(planId) as {
      connection_id: string;
      plan_kind: CleanupPlan["kind"];
    ...
    if (planScope.plan_kind === "organize") {
      const current = this.#database
        ...
        .get(this.#profileId, planScope.connection_id) as
        { id: string; revision: string } | undefined;
      if (
        !current ||
        current.id !== planScope.proposal_id ||
        current.revision !== planScope.proposal_revision
      )
        throw new Error("cleanup_plan_changed");
    }
    ...
    const legacyContainers = this.#database
      .prepare(
        "SELECT id FROM cleanup_legacy_containers WHERE plan_id=? ORDER BY rowid",
      )
      .all(planId) as Array<{ id: string }>;
  ...
  action(actionId: string): CleanupActionRecord {
    ...
    return {
      ...
      priorFlags: row.prior_flags_json
        ? (JSON.parse(String(row.prior_flags_json)) as string[])
        : [],
      markRead: Boolean(row.mark_read),
      resultingPath: row.resulting_path ? String(row.resulting_path) : null,
      resultingUidValidity: row.resulting_uid_validity
        ? String(row.resulting_uid_validity)
        : null,
  ...
  targets(planId: string): CleanupTargetRecord[] {
    ...
    `,
        ...
        .all(planId) as Array<{
        target_path: string;
        action_kind: CleanupActionRecord["actionKind"];
      }>
    ).map((row) => ({
    ...
    this.#database.transaction(() => {
      ...
      `,
        ...
        .run(
          ...
          receipt?.uid ?? null,
          receipt ? JSON.stringify([...receipt.flags].sort()) : null,
          timestamp,
          actionId,
        );
  ...
  markMovePending(
    actionId: string,
    pointer: { path: string; uid: number },
  ): void {
    this.#database
      .prepare(
        `
    ...
    if (!occupied || occupied.id === action.messageRowId) {
      this.#database
        ...
        .run(
          ...
          receipt.uidValidity,
          receipt.uid,
          JSON.stringify([...receipt.flags].sort()),
          timestamp,
          action.messageRowId,
          action.connectionId,
        );
  ...
  retry(planId: string, actionIds: readonly string[]): CleanupPlan {
    this.assertMutableSources(planId);
    const plan = this.get(planId);
    ...
    this.#database
      .prepare("UPDATE cleanup_plans SET state='approved' WHERE id=?")
      .run(planId);
    return this.get(planId);
  }
  ...
  prepareUndo(planId: string): CleanupPlan {
    ...
    if (plan.job?.state !== "succeeded")
      throw new Error("cleanup_undo_not_ready");
    const actions = this.#database
      .prepare(
        `
    ...
    this.#database.transaction(() => {
      this.#database
        .prepare("UPDATE cleanup_plans SET undo_job_id=? WHERE id=?")
        .run(job.id, planId);
      this.#database
        .prepare(
          "UPDATE cleanup_actions SET undo_state='pending',undo_error_code=NULL WHERE plan_id=? AND state='succeeded'",
        )
        .run(planId);
    })();
    return this.get(planId);
  ...
  markUndo(
    ...
    this.#database
      .prepare(
        "UPDATE cleanup_actions SET undo_state=?,undo_error_code=?,updated_at=? WHERE id=?",
      )
      ...
      plan.job?.state === "succeeded"
        ...
        : plan.job &&
          ...
          : plan.job?.state === "running"
            ...
            : plan.state;
    this.#database
      .prepare("UPDATE cleanup_plans SET state = ? WHERE id = ?")
      .run(state, planId);
    return this.get(planId);
  }
}
