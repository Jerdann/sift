...
export class MailboxAnalysisRepository {
  ...
  save(
    ...
    items: readonly StoredClassification[],
  ): MailboxAnalysisSummary {
    const analysisId =
      (
        this.#database
          .prepare(
            "SELECT id FROM mailbox_analyses WHERE connection_id=? ORDER BY rowid DESC LIMIT 1",
          )
    ...
    for (const item of items) {
      for (const address of item.receivingAddresses.length
        ? item.receivingAddresses
        : ["unknown"]) {
        ...
        current.confidence =
          (current.confidence * current.messageCount + item.confidence) /
          (current.messageCount + 1);
        current.messageCount += 1;
        if (
          item.receivedAt &&
          (!current.latestAt || item.receivedAt > current.latestAt)
        )
          current.latestAt = item.receivedAt;
        current.evidence = [
    ...
    this.#database.transaction(() => {
      ...
      this.#database
        ...
        VALUES (?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET classifier_version=excluded.classifier_version,analyzed_at=excluded.analyzed_at
      `,
        )
        .run(
          analysisId,
      ...
      for (const item of items) {
        addClassification.run(
          ...
          item.senderDomain,
          JSON.stringify(item.receivingAddresses),
        );
      }
      ...
      for (const stream of streams.values()) {
        ...
        if (
          stream.receivingAddress === "unknown" ||
          stream.senderDomain === "unknown-sender"
        )
          continue;
        const key = `${stream.receivingAddress}\0${stream.senderDomain}`;
        const service = services.get(key) ?? {
          address: stream.receivingAddress,
          domain: stream.senderDomain,
          count: 0,
          latestAt: null,
          categories: new Set(),
        };
        ...
        if (
          stream.latestAt &&
          (!service.latestAt || stream.latestAt > service.latestAt)
        )
          service.latestAt = stream.latestAt;
      ...
      for (const service of services.values()) {
        addService.run(
          ...
          service.count,
          service.latestAt,
          JSON.stringify([...service.categories].sort()),
        );
      }
    })();
  ...
  get(connectionId: string): MailboxAnalysisSummary | null {
    ...
    `,
      ...
      .get(connectionId, this.#profileId) as
    ...
    const identityRepository = new AccountIdentityRepository(
      this.#database,
      this.#profileId,
    );
    ...
    if (!identities.length) {
      identities = identityRepository.sync(
        "proton",
        connectionId,
        protonIdentityEvidence(this.#database, connectionId),
      );
    }
    const ownership = identities.filter(
    ...
    const safeStreams = streams.filter((stream) =>
      verifiedAddresses.has(stream.receiving_address),
    );
    const addresses = ownership
      .map((identity) => {
        const address = identity.address;
        const rows = serviceRows.filter(
          (row) => row.receiving_address === address,
        );
        const messageCount = rows.reduce(
          (sum, row) => sum + row.message_count,
          0,
        );
        const latestAt =
          rows
      ...
      `,
            ...
            .get(analysis.id, address) as { count: number }
        ).count;
        const ageDays = latestAt
          ? (Date.parse(this.#now()) - Date.parse(latestAt)) / 86_400_000
          : Number.POSITIVE_INFINITY;
        let recommendation:
          "retain" | "migrate" | "watch" | "consider_deactivation";
        let rationale: string;
        if (importantCount > 0) {
          recommendation = "retain";
          rationale = `${importantCount} account, security, transaction, or finance messages make this address important.`;
        } else if (
          identity.sentFromCount > 0 &&
          (messageCount === 0 || ageDays > 730)
        ) {
          recommendation = "consider_deactivation";
          rationale =
            "This verified sending identity has no recent important service evidence; verify manually before retirement.";
        } else if (
          rows.some((row) =>
            (JSON.parse(row.categories_json) as MailCategory[]).some(
              (category) => ["games", "shopping", "social"].includes(category),
            ),
          )
        ) {
          recommendation = "migrate";
          rationale =
            "Active service identities were found, but no critical account mail; consider consolidating them.";
        } else {
          recommendation = "watch";
          rationale =
            "Recent activity exists, but the evidence is not strong enough for a retain or deactivate decision.";
        }
        return {
          address,
          ownershipEvidence:
    ...
    return mailboxAnalysisSummarySchema.parse({
      ...
      uniqueMessages: categories.reduce(
        (sum, category) => sum + category.message_count,
        0,
      ),
