...
export interface ProtonMutationClientPort {
  ...
  inspect(
    sourcePath: string,
    uid: number,
  ): Promise<{ uidValidity: string; flags: string[] } | null>;
  inspectMany(
    sourcePath: string,
    uids: readonly number[],
  ): Promise<Map<number, { uidValidity: string; flags: string[] }>>;
  moveMany(
    sourcePath: string,
    uids: readonly number[],
    targetPath: string,
    markRead?: boolean,
  ): Promise<Map<number, ProtonMovePointer>>;
  restore(
    targetPath: string,
    uid: number,
    sourcePath: string,
    priorFlags: readonly string[],
  ): Promise<ProtonMoveReceipt | null>;
...
export class ImapFlowMutationClient implements ProtonMutationClientPort {
  ...
  async prepareTarget(
    ...
    if (nativeSpam) {
      const junk = mailboxes.find(
        (mailbox) => mailbox.specialUse?.toLowerCase() === "\\junk",
      );
    ...
    if (nativeTrash) {
      const trash = mailboxes.find(
        (mailbox) => mailbox.specialUse?.toLowerCase() === "\\trash",
      );
    ...
    const existing = mailboxes.find(
      (mailbox) => mailbox.path.toLowerCase() === providerPath.toLowerCase(),
    );
    if (existing) return existing.path;
    const parts = providerPath.split(delimiter).filter(Boolean);
    const foldersRoot = mailboxes.find(
      (mailbox) => mailbox.path.toLowerCase() === parts[0]!.toLowerCase(),
    );
    ...
    for (let index = 2; index <= parts.length; index += 1) {
      ...
      if (
        !mailboxes.some(
          (mailbox) => mailbox.path.toLowerCase() === current.toLowerCase(),
        )
      ) {
    ...
    const verified = mailboxes.find(
      (mailbox) => mailbox.path.toLowerCase() === providerPath.toLowerCase(),
    );
    if (!verified) throw new Error("folder_verification_failed");
  ...
  async inspectMany(
    ...
    const lock = await this.#client.getMailboxLock(sourcePath, {
      readOnly: true,
      description: "Sift prior-state capture",
    });
    try {
  ...
  async moveMany(
    ...
    const lock = await this.#client.getMailboxLock(sourcePath, {
      description: "Sift approved cleanup",
    });
    try {
      if (markRead) {
        const seen = await this.#client.messageFlagsAdd([...uids], ["\\Seen"], {
          uid: true,
        });
        if (!seen) throw new Error("provider_seen_rejected");
      }
      if (sourcePath.toLowerCase() === targetPath.toLowerCase()) {
        for (const uid of uids) pointers.set(uid, { path: sourcePath, uid });
        return pointers;
      }
      const moved = await this.#client.messageMove([...uids], targetPath, {
        uid: true,
      });
    ...
    for (const sourceUid of uids) {
      ...
      if (targetUid)
        pointers.set(sourceUid, { path: targetPath, uid: targetUid });
    }
    return pointers;
  }
  ...
  async restore(
    ...
    const targetLock = await this.#client.getMailboxLock(targetPath, {
      description: "Sift cleanup undo",
    });
    try {
      if (targetPath.toLowerCase() === sourcePath.toLowerCase())
        sourceUid = uid;
      else {
        const moved = await this.#client.messageMove(uid, sourcePath, {
          uid: true,
        });
    ...
    const sourceLock = await this.#client.getMailboxLock(sourcePath, {
      description: "Sift cleanup flag restore",
    });
    try {
      const restored = await this.#client.messageFlagsSet(
        sourceUid,
        [...priorFlags],
        { uid: true },
      );
    ...
    return resulting
      ? { path: sourcePath, uid: sourceUid, ...resulting }
      : null;
  }

  async retireContainer(
    path: string,
  ): Promise<"retired" | "missing" | "not_empty"> {
    ...
    if (
      mailbox.specialUse ||
      parts.length < 2 ||
      !["folders", "labels"].includes(parts[0]!.toLowerCase())
    ) {
    ...
    return remaining.some(
      (candidate) =>
        candidate.path.toLowerCase() === mailbox.path.toLowerCase(),
    )
      ? "not_empty"
      : "retired";
  }
...
export const createProtonMutationClient: ProtonMutationClientFactory = async (
  credentials,
) => {
  const { ImapFlow: RuntimeImapFlow } = await import("imapflow");
  return new ImapFlowMutationClient(
    new RuntimeImapFlow(bridgeOptionsFor(credentials)),
  );
};
