...
export const registerGmailHandlers = ({ ipcMain, profileSession, developmentServerUrl, fetchPort = fetch }: { ipcMain: IpcMain; profileSession: ProfileSession; developmentServerUrl?: string; fetchPort?: OAuthFetch }): (() => void) => {
  ...
  ipcMain.handle(IPC_CHANNELS.gmailSpamGet,(event)=>{trust(event);const current=organization();const connection=current.connection.get();return gmailOrganizationPlanSchema.nullable().parse(connection?current.repository.get(connection.id,'spam'):null);});
  ipcMain.handle(IPC_CHANNELS.gmailSpamGenerate,(event)=>{trust(event);const current=organization();const connection=current.connection.get();if(!connection)throw new Error('gmail_not_connected');return gmailOrganizationPlanSchema.parse(current.repository.generate(connection,{kind:'spam'}));});
  ...
  return () => {
    ...
    ipcMain.removeHandler(IPC_CHANNELS.gmailSpamGet);
    ipcMain.removeHandler(IPC_CHANNELS.gmailSpamGenerate);
