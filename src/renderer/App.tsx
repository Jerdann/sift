...
const TrashReviewPanel = ({
  ...
  if (!analysis) return null;
  const candidates = rankStaleStreams(analysis.topStreams).map((candidate) => ({ ...candidate, age: recency(candidate.latestAt) }));
  return <>
...
const UnsubscribePanel = ({
  ...
  const manualCount = dashboard?.candidates.filter((candidate) => candidate.eligibility === 'manual').length ?? 0;
    ...
    <section className="readiness-panel unsubscribe-panel" aria-labelledby={`${provider}-unsubscribe-title`}>
      ...
      {!dashboard ? (
        ...
        <div className="unsubscribe-review">
          <div className="unsubscribe-summary"><span><b>{eligible.length}</b> safe one-click candidates</span><span><b>{manualCount}</b> need manual action</span><span><b>{protectedCount}</b> protected lists</span><span><b>{spamCount}</b> spam contacts blocked</span><button className="secondary-button" type="button" disabled={busy || runActive} onClick={() => void act(async () => { await onScan(); setSelected([]); setConsent(false); })}>Refresh</button></div>
          <div className="unsubscribe-list">
            {eligible.map((candidate) => (
              <label key={candidate.id}><input type="checkbox" checked={selected.includes(candidate.id)} onChange={(event) => setSelected((current) => event.target.checked ? [...current, candidate.id] : current.filter((id) => id !== candidate.id))} /><span><strong>{candidate.senderDomain}</strong><small>{candidate.messageCount.toLocaleString()} messages · {candidate.messagesPerMonth.toFixed(1)}/month · {Math.round(candidate.readRate*100)}% read → {candidate.receivingAddress} · {candidate.sampleSubjects[0] ?? candidate.listId}</small></span><b>{candidate.recurrence === 'recurring' ? 'RECURRING' : `SCORE ${Math.round(candidate.priorityScore)}`}</b></label>
            ))}
          ...
          {(['manual','protected','spam_skipped'] as const).map((eligibility)=><details className="unsubscribe-exclusions" key={eligibility}><summary>{eligibility==='manual'?`Manual action queue (${manualCount})`:eligibility==='protected'?`Protected mail (${protectedCount})`:`Spam contacts blocked (${spamCount})`}</summary><ul>{dashboard.candidates.filter(candidate=>candidate.eligibility===eligibility).slice(0,100).map(candidate=><li key={candidate.id}><span><strong>{candidate.senderDomain}</strong><small>{candidate.reason} · {candidate.receivingAddress}</small></span><b>{candidate.messageCount.toLocaleString()}</b></li>)}</ul></details>)}
        </div>
      )}
      {error ? <p className="connection-error" role="alert">{error}</p> : null}
    </section>
...
const GmailOrganizationPanel=({analysis,plan,onGenerate,onApprove,onRetry,onUndo,mode='organize'}:{analysis:MailboxAnalysisSummary|null;plan:GmailOrganizationPlan|null;onGenerate():Promise<void>;onApprove(id:string,revision:string):Promise<void>;onRetry(id:string,batchIds:string[]):Promise<void>;onUndo(id:string):Promise<void>;mode?:'organize'|'trash'})=>{const[consent,setConsent]=useState(false);const[busy,setBusy]=useState(false);const[error,setError]=useState('');if(!analysis)return null;const trash=mode==='trash';const act=async(action:()=>Promise<void>)=>{setBusy(true);setError('');try{await action();}catch{setError('Gmail history stopped safely at a verified batch checkpoint. Retry failed work or rebuild the plan after a mailbox-state mismatch.');}finally{setBusy(false);}};const progress=plan?.undoJob??plan?.job;return <section className="readiness-panel gmail-organize-panel" aria-labelledby={`gmail-${mode}-title`}><div className="panel-header"><div><p className="eyebrow">{trash?'GMAIL STALE HISTORY':'GMAIL HISTORY'}</p><h2 id={`gmail-${mode}-title`}>{trash?'Move the approved old mail to Gmail Trash':'Apply the approved labels to existing Gmail'}</h2></div><span className="secured-label"><ShieldCheck size={14}/> Verified batches</span></div>{!plan?<div className="analysis-empty"><p>{trash?'Build reversible 100-message Trash batches from the selected stale senders. Every batch verifies live labels before and after the move.':'Build 100-message batches from the corrected address-scoped proposal. Every batch checks live labels before changing anything and re-reads every message afterward.'}</p><button className="primary-button compact" disabled={busy} onClick={()=>void act(onGenerate)}>{busy?'Building batches…':trash?'Preview Gmail Trash impact':'Preview Gmail history impact'}</button></div>:<div className="cleanup-review"><div className="cleanup-totals"><div><b>{plan.impactCount}</b><span>{trash?'sender impacts':'address/category impacts'}</span></div><div><b>{plan.existingMessageCount.toLocaleString()}</b><span>existing messages</span></div><div><b>{plan.skippedAmbiguousStreams}</b><span>protected or uncertain</span></div></div><div className="proposal-table"><div className="cleanup-impact-row cleanup-impact-head"><span>Address / source</span><span>Label</span><span>Action</span><span>Messages</span></div>{plan.impacts.slice(0,100).map(impact=><div className="cleanup-impact-row" key={impact.id}><strong>{impact.scopeAddress??impact.sourceCategory}</strong><span>{impact.targetLabel}</span><small>{impact.trash?'Move to Trash':impact.spam?'Move to Spam':`${impact.markRead?'read · ':''}${impact.archive?'archive':'keep in inbox'}`}</small><b>{impact.existingMessages}</b></div>)}</div>{plan.state==='draft'?<div className="cleanup-approval"><label><input type="checkbox" checked={consent} onChange={event=>setConsent(event.target.checked)}/><span><strong>I approve these exact Gmail history changes</strong><small>{trash?'Only the listed old, non-critical messages move to Gmail Trash. Exact prior labels are retained for Undo.':'Future filters are managed separately on Rules. This pass changes only the listed existing messages and keeps exact prior labels for Undo.'}</small></span></label><button className="primary-button" disabled={!consent||busy||!plan.impactCount} onClick={()=>void act(()=>onApprove(plan.id,plan.revision))}>{busy?'Applying and verifying…':trash?'Move approved mail to Gmail Trash':'Apply approved Gmail history'}</button></div>:<div className="cleanup-execution"><div><span><strong>{plan.undoJob?.state==='succeeded'?'Original Gmail labels restored':plan.state==='completed'?(trash?'Gmail Trash moves verified':'Gmail history verified'):plan.state==='failed'?'Some batches need attention':trash?'Moving Gmail history to Trash':'Applying Gmail history'}</strong><small>{progress?.completedItems??0} / {progress?.totalItems??plan.batchCount} batches processed</small></span><b>{progress?.percent??0}%</b></div><progress max={100} value={progress?.percent??0}/><div className="cleanup-job-actions">{plan.failedBatches.length?<button className="primary-button compact" disabled={busy} onClick={()=>void act(()=>onRetry(plan.id,plan.failedBatches.map(batch=>batch.id)))}>Retry {plan.failedBatches.length} failed</button>:null}{plan.job?.state==='succeeded'&&!plan.undoJob?<button className="secondary-button danger" disabled={busy} onClick={()=>void act(()=>onUndo(plan.id))}>Undo verified batches</button>:null}</div></div>}</div>}{error?<p className="connection-error">{error}</p>:null}</section>;};

const GmailTrashReviewPanel=({analysis,plan,onGenerate,onApprove,onRetry,onUndo}:{analysis:MailboxAnalysisSummary;plan:GmailOrganizationPlan|null;onGenerate(senderDomains:string[]):Promise<void>;onApprove(id:string,revision:string):Promise<void>;onRetry(id:string,batchIds:string[]):Promise<void>;onUndo(id:string):Promise<void>})=>{const[selected,setSelected]=useState<string[]>([]);const candidates=rankStaleStreams(analysis.topStreams).map(candidate=>({...candidate,age:recency(candidate.latestAt)}));return <><section className="readiness-panel trash-review-panel"><div className="panel-header"><div><p className="eyebrow">GMAIL STALE HISTORY</p><h2>Choose old Gmail sender streams to remove</h2></div><span className="secured-label"><Archive size={14}/> Reversible Trash only</span></div><div className="trash-review-copy"><p>Only mail older than six months is eligible. Critical classifications and anything already in Spam or Trash stay out of the plan.</p><span><b>{candidates.reduce((sum,item)=>sum+item.messages,0).toLocaleString()}</b> removable messages across <b>{candidates.length}</b> stale senders</span></div><div className="trash-candidates"><div className="trash-candidate-head"><span></span><span>Sender</span><span>Alias scope</span><span>Newest message</span><span>Protected</span><span>Removable</span></div>{candidates.slice(0,200).map(candidate=><label key={candidate.domain}><input type="checkbox" checked={selected.includes(candidate.domain)} onChange={event=>setSelected(current=>event.target.checked?[...current,candidate.domain]:current.filter(domain=>domain!==candidate.domain))}/><strong>{candidate.domain}</strong><span>{candidate.addresses.length===1?candidate.addresses[0]:`${candidate.addresses.length} aliases`}</span><small>{candidate.age.label}</small><b>{candidate.protected.toLocaleString()}</b><em>{candidate.messages.toLocaleString()}</em></label>)}</div>{!candidates.length?<p className="analysis-empty-note">No non-critical Gmail sender stream is old enough for this pass.</p>:null}{candidates.length?<div className="trash-selection"><button className="secondary-button" type="button" onClick={()=>setSelected(selected.length===candidates.length?[]:candidates.map(candidate=>candidate.domain))}>{selected.length===candidates.length?'Clear selection':'Select all stale senders'}</button><span>{selected.length} sender{selected.length===1?'':'s'} selected</span></div>:null}</section>{selected.length||plan?<GmailOrganizationPanel analysis={analysis} plan={plan} onGenerate={()=>onGenerate(selected)} onApprove={onApprove} onRetry={onRetry} onUndo={onUndo} mode="trash"/>:null}</>};
    ...
    <div className="app-shell">
      ...
      <div className="workspace">
        ...
        <main className="overview">
          ...
          {activePage === 'delete' ? <>{taskIntro('Delete last, when the broad cleanup work is already done.', 'Review stale sender history by volume and last activity, protect critical classifications, then move only the exact approved messages into each provider’s recoverable Trash.')}{!analysis && !gmailAnalysis ? prerequisite('Build an organization proposal first', 'The final deletion pass depends on proven aliases and classified sender history.', scannedCount ? 'organize' : 'audit', scannedCount ? 'Open organize' : 'Open scan') : <>{gmailAnalysis ? <GmailTrashReviewPanel analysis={gmailAnalysis} plan={gmailDeletion} onGenerate={onGenerateGmailDeletion} onApprove={onApproveGmailOrganization} onRetry={onRetryGmailOrganization} onUndo={onUndoGmailOrganization}/> : null}{analysis ? <TrashReviewPanel analysis={analysis} plan={deletionPlan} onGenerate={onGenerateDeletion} onApprove={onApproveDeletion} onResume={onResumeDeletion} onRetry={onRetryDeletion} onUndo={onUndoDeletion} /> : null}</>}</> : null}
        </main>
      </div>
    </div>
  );
...
export const App = () => {
  ...
  const [gmailDeletion,setGmailDeletion]=useState<GmailOrganizationPlan|null>(null);
  ...
  const loadWorkspace = async () => {
    const [connection, discovery, audit, mailboxAnalysis, currentCleanupPlan, currentDeletionPlan, currentSubscriptions, currentGmail, currentGmailAudit, currentGmailAnalysis, currentGmailOrganization, currentGmailDeletion, currentGmailSubscriptions, currentAccounts] = await Promise.all([
    ...
    setGmailOrganization(currentGmailOrganization); setGmailDeletion(currentGmailDeletion); setGmailSubscriptions(currentGmailSubscriptions); setAccounts(currentAccounts);
  ...
  const setGmailHistoryPlan=(plan:GmailOrganizationPlan)=>plan.kind==='trash'?setGmailDeletion(plan):setGmailOrganization(plan);
  const generateGmailOrganization=async()=>setGmailHistoryPlan(await window.emailOrganizer.generateGmailOrganizationPlan());
  const generateGmailDeletion=async(senderDomains:string[])=>setGmailHistoryPlan(await window.emailOrganizer.generateGmailDeletionPlan({senderDomains,olderThanDays:180}));
  const approveGmailOrganization=async(planId:string,revision:string)=>setGmailHistoryPlan(await window.emailOrganizer.approveGmailOrganizationPlan({planId,revision}));
  const retryGmailOrganization=async(planId:string,batchIds:string[])=>setGmailHistoryPlan(await window.emailOrganizer.retryGmailOrganizationPlan({planId,batchIds}));
  const undoGmailOrganization=async(planId:string)=>setGmailHistoryPlan(await window.emailOrganizer.undoGmailOrganizationPlan({planId}));
