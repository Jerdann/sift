...
const GmailOrganizationPanel=({analysis,plan,onGenerate,onApprove,onRetry,onUndo}:{analysis:MailboxAnalysisSummary|null;plan:GmailOrganizationPlan|null;onGenerate():Promise<void>;onApprove(id:string,revision:string):Promise<void>;onRetry(id:string,batchIds:string[]):Promise<void>;onUndo(id:string):Promise<void>})=>{const[consent,setConsent]=useState(false);const[busy,setBusy]=useState(false);const[error,setError]=useState('');if(!analysis)return null;const act=async(action:()=>Promise<void>)=>{setBusy(true);setError('');try{await action();}catch{setError('Gmail history stopped safely at a verified batch checkpoint. Retry failed work or rebuild the plan after a mailbox-state mismatch.');}finally{setBusy(false);}};const progress=plan?.undoJob??plan?.job;return <section className="readiness-panel gmail-organize-panel" aria-labelledby="gmail-organize-title"><div className="panel-header"><div><p className="eyebrow">GMAIL HISTORY</p><h2 id="gmail-organize-title">Apply the approved labels to existing Gmail</h2></div><span className="secured-label"><ShieldCheck size={14}/> Verified batches</span></div>{!plan?<div className="analysis-empty"><p>Build 100-message batches from the corrected address-scoped proposal. Every batch checks live labels before changing anything and re-reads every message afterward.</p><button className="primary-button compact" disabled={busy} onClick={()=>void act(onGenerate)}>{busy?'Building batches…':'Preview Gmail history impact'}</button></div>:<div className="cleanup-review"><div className="cleanup-totals"><div><b>{plan.impactCount}</b><span>address/category impacts</span></div><div><b>{plan.existingMessageCount.toLocaleString()}</b><span>existing messages</span></div><div><b>{plan.skippedAmbiguousStreams}</b><span>protected or uncertain</span></div></div><div className="proposal-table"><div className="cleanup-impact-row cleanup-impact-head"><span>Address / source</span><span>Label</span><span>Action</span><span>Messages</span></div>{plan.impacts.slice(0,100).map(impact=><div className="cleanup-impact-row" key={impact.id}><strong>{impact.scopeAddress??impact.sourceCategory}</strong><span>{impact.targetLabel}</span><small>{impact.spam?'Move to Spam':`${impact.markRead?'read · ':''}${impact.archive?'archive':'keep in inbox'}`}</small><b>{impact.existingMessages}</b></div>)}</div>{plan.state==='draft'?<div className="cleanup-approval"><label><input type="checkbox" checked={consent} onChange={event=>setConsent(event.target.checked)}/><span><strong>I approve these exact Gmail history changes</strong><small>Future filters are managed separately on Rules. This pass changes only the listed existing messages and keeps exact prior labels for Undo.</small></span></label><button className="primary-button" disabled={!consent||busy||!plan.impactCount} onClick={()=>void act(()=>onApprove(plan.id,plan.revision))}>{busy?'Applying and verifying…':'Apply approved Gmail history'}</button></div>:<div className="cleanup-execution"><div><span><strong>{plan.undoJob?.state==='succeeded'?'Original Gmail labels restored':plan.state==='completed'?'Gmail history verified':plan.state==='failed'?'Some batches need attention':'Applying Gmail history'}</strong><small>{progress?.completedItems??0} / {progress?.totalItems??plan.batchCount} batches processed</small></span><b>{progress?.percent??0}%</b></div><progress max={100} value={progress?.percent??0}/><div className="cleanup-job-actions">{plan.failedBatches.length?<button className="primary-button compact" disabled={busy} onClick={()=>void act(()=>onRetry(plan.id,plan.failedBatches.map(batch=>batch.id)))}>Retry {plan.failedBatches.length} failed</button>:null}{plan.job?.state==='succeeded'&&!plan.undoJob?<button className="secondary-button danger" disabled={busy} onClick={()=>void act(()=>onUndo(plan.id))}>Undo verified batches</button>:null}</div></div>}</div>}{error?<p className="connection-error">{error}</p>:null}</section>;};
...
export const App = () => {
  ...
  const retryGmailOrganization=async(planId:string,batchIds:string[])=>setGmailOrganization(await window.emailOrganizer.retryGmailOrganizationPlan({planId,batchIds}));
